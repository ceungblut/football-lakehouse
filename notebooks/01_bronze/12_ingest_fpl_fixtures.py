...
dbutils.widgets.text("source_url", "https://fantasy.premierleague.com/api/fixtures/", "source_url")
dbutils.widgets.text("run_id", str(uuid.uuid4()), "run_id (optional)")
...
resp = requests.get(source_url,headers=headers,timeout=(float(timeout_seconds), float(timeout_seconds)))
...
table_name = "football.bronze.fpl_fixtures_raw"
...
spark.sql(f"""
    SELECT 1
    FROM football.bronze.fpl_fixtures_raw""").display()
...
exists = spark.sql(f"""
    SELECT 1
    FROM football.bronze.fpl_fixtures_raw
    WHERE 
...
# MAGIC FROM football.bronze.fpl_fixtures_raw
